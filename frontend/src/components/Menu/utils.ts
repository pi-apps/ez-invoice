...
export const getActiveSubMenuItem = ({ pathname, menuItem }: { pathname: string; menuItem?: ConfigMenuItemsType }) => {
  const activeSubMenuItems = menuItem?.items ? menuItem?.items.filter((subMenuItem) => pathname.startsWith(subMenuItem?.href ? subMenuItem?.href : "" )) : []
  ...
  const mostSpecificMatch = activeSubMenuItems.sort(
    (subMenuItem1, subMenuItem2) => subMenuItem2?.href && subMenuItem1?.href ? subMenuItem2?.href.length - subMenuItem1?.href.length : 0 ,
  )[0]
