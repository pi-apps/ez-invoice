...
export default function mountInvoiceEndpoints(router: Router) {
    ...
    router.post('/create', upload.single("logo"), async (req, res) => {
        try {
            ...
            const amountDiscount = discountType == 1 ? new BigNumber(amountTax).multipliedBy(discount).dividedBy(100) : new BigNumber(discount);            
            const total = new BigNumber(amountTax).minus(amountDiscount).plus(shipping);
